  ...
  return (
    <main className="bg-white">
      <section className="relative overflow-hidden bg-gradient-to-br from-[#01547a] via-[#013f5c] to-[#002d42] px-6 py-24 text-white lg:px-8 lg:py-32">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,rgba(245,197,24,0.12),transparent_55%)]" />
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-blue-100">Questions about a shipment, business delivery, franchise or coverage? Send us a message.</p>
        </div>
      </section>
      ...
      <section className="px-6 py-14 lg:px-8 lg:py-20 bg-gray-50">
        <div className="mx-auto grid max-w-6xl gap-7 lg:grid-cols-[.75fr_1.25fr]">
          <div className="rounded-2xl bg-white border border-gray-200 p-8 text-gray-900 sm:p-9">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#027196]">Get in touch</p>
            <h2 className="site-display mt-4 text-3xl font-extrabold text-gray-900">We are here to help.</h2>
...
function Info({ Icon, title, text }) {
  return (
    <div className="flex gap-4">
      <div className="rounded-xl bg-[#027196]/12 p-3 text-[#027196]"><Icon className="h-5 w-5" /></div>
      <div>
        <p className="text-xs font-bold uppercase tracking-wide text-gray-600">{title}</p>
        <p className="mt-1 text-sm font-semibold text-gray-800">{text}</p>
      </div>
    </div>
  );
}
