"use client";

import dynamic from "next/dynamic";
import { useCallback, useEffect, useMemo, useState } from "react";

import {
  Badge,
  Button,
  Card,
  Col,
  Descriptions,
  Empty,
  Input,
  InputNumber,
  Pagination,
  Popconfirm,
  Row,
  Space,
  Statistic,
  Switch,
  Tabs,
  Tag,
  Tooltip,
  Typography,
  message,
} from "antd";

import {
  CheckCircleOutlined,
  CloseOutlined,
  DeleteOutlined,
  EditOutlined,
  EnvironmentOutlined,
  PlusOutlined,
  ReloadOutlined,
  SaveOutlined,
  SwapOutlined,
  WarningOutlined,
} from "@ant-design/icons";

import {
  createBranchRouteRate,
  deleteBranchRouteRate,
  getBranchRouteRateMatrix,
  updateBranchRouteRate,
  updateBranchRouteRateStatus,
} from "@/services/adminRateManagementService";

import { getPricingSettings } from "@/services/adminPricingConfigurationService";

import {
  apiErrorMessage,
  formatDate,
  formatMoney,
  normalizeBranch,
  normalizeBranchRate,
} from "@/lib/rate-management-page-utils";

const { Title, Text } = Typography;

const RouteMapS = dynamic(
  () => import("@/components/rate-admin/RouteMapS"),
  {
    ssr: false,
    loading: () => (
      <div
        style={{
          height: 280,
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          background: "#f5f7fa",
          borderRadius: 8,
          color: "#8c8c8c",
        }}
      >
        Loading map...
      </div>
    ),
  }
);

/*
|--------------------------------------------------------------------------
| Helpers
|--------------------------------------------------------------------------
*/

function toBoolean(value, fallback = false) {
  if (value === true || value === 1 || value === "1" || value === "true") {
    return true;
  }

  if (
    value === false ||
    value === 0 ||
    value === "0" ||
    value === "false" ||
    value === null ||
    value === undefined
  ) {
    return fallback;
  }

  return Boolean(value);
}

function toNumber(value, fallback = 0) {
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}

function getPickupId(rate) {
  return toNumber(
    rate?.pickup_coverage_location_id ?? rate?.pickup_branch_id,
    0
  );
}

function getDeliveryId(rate) {
  return toNumber(
    rate?.delivery_coverage_location_id ?? rate?.delivery_branch_id,
    0
  );
}

/*
|--------------------------------------------------------------------------
| Normalize a rate for local state
|--------------------------------------------------------------------------
|
| IMPORTANT:
| The API response can sometimes omit fields that were included in the
| update request. We therefore merge the submitted values with the response
| before normalizing.
|
*/

function normalizeLocalRate(rate, fallback = {}) {
  const merged = {
    ...fallback,
    ...(rate || {}),
  };

  return normalizeBranchRate({
    ...merged,

    pickup_coverage_location_id:
      merged.pickup_coverage_location_id ??
      merged.pickup_branch_id ??
      fallback.pickup_coverage_location_id,

    delivery_coverage_location_id:
      merged.delivery_coverage_location_id ??
      merged.delivery_branch_id ??
      fallback.delivery_coverage_location_id,

    base_rate:
      merged.base_rate ??
      fallback.base_rate ??
      0,

    is_active: toBoolean(
      merged.is_active,
      toBoolean(fallback.is_active, true)
    ),

    express_enabled: toBoolean(
      merged.express_enabled,
      toBoolean(fallback.express_enabled, true)
    ),

    same_day_enabled: toBoolean(
      merged.same_day_enabled,
      toBoolean(fallback.same_day_enabled, true)
    ),
  });
}

/*
|--------------------------------------------------------------------------
| Extract API object
|--------------------------------------------------------------------------
|
| Supports responses such as:
|
| {
|   id: 10,
|   ...
| }
|
| {
|   data: {
|     id: 10,
|     ...
|   }
| }
|
| {
|   success: true,
|   data: {
|     id: 10,
|     ...
|   }
| }
|
*/

function extractApiObject(response) {
  if (!response) {
    return null;
  }

  if (response?.data?.data && typeof response.data.data === "object") {
    return response.data.data;
  }

  if (response?.data && typeof response.data === "object") {
    return response.data;
  }

  if (typeof response === "object") {
    return response;
  }

  return null;
}

/*
|--------------------------------------------------------------------------
| Status tag
|--------------------------------------------------------------------------
*/

function StatusTag({ active }) {
  return (
    <Tag
      icon={active ? <CheckCircleOutlined /> : null}
      color={active ? "success" : "default"}
      style={{
        marginInlineEnd: 0,
        borderRadius: 6,
      }}
    >
      {active ? "Active" : "Inactive"}
    </Tag>
  );
}

/*
|--------------------------------------------------------------------------
| Empty inline form
|--------------------------------------------------------------------------
*/

function emptyForm() {
  return {
    base_rate: 0,
    is_active: true,
    express_enabled: true,
    same_day_enabled: true,
  };
}

/*
|--------------------------------------------------------------------------
| Branch Pricing Page
|--------------------------------------------------------------------------
*/

export default function BranchPricingPage() {
  const [locations, setLocations] = useState([]);
  const [rateMap, setRateMap] = useState({});
  const [selectedPickupId, setSelectedPickupId] = useState(null);
  const [selectedRoute, setSelectedRoute] = useState(null);

  const [activePricingSettings, setActivePricingSettings] =
    useState(null);

  const [loading, setLoading] = useState(false);

  const [search, setSearch] = useState("");
  const [destTab, setDestTab] = useState("all");

  const [branchSearch, setBranchSearch] = useState("");
  const [branchTab, setBranchTab] = useState("all");
  const [branchPage, setBranchPage] = useState(1);

  const BRANCH_PAGE_SIZE = 10;

  /*
  |--------------------------------------------------------------------------
  | Inline state
  |--------------------------------------------------------------------------
  */

  const [inlineAdd, setInlineAdd] = useState(null);
  const [inlineEdit, setInlineEdit] = useState(null);

  const [inlineForm, setInlineForm] = useState(emptyForm());

  const [inlineSaving, setInlineSaving] = useState(false);

  /*
  |--------------------------------------------------------------------------
  | LOAD MATRIX
  |--------------------------------------------------------------------------
  */

  const loadMatrix = useCallback(
    async (keepSelection = false) => {
      try {
        setLoading(true);

        const payload = await getBranchRouteRateMatrix();

        const data = payload?.data ?? payload;

        const rawLocations = Array.isArray(data?.branches)
          ? data.branches
          : [];

        const rawRates = data?.rates ?? {};

        const normalizedLocations =
          rawLocations.map(normalizeBranch);

        setLocations(normalizedLocations);

        const normalizedMap = {};

        for (const [key, rate] of Object.entries(rawRates)) {
          normalizedMap[key] = normalizeLocalRate(rate);
        }

        setRateMap(normalizedMap);

        if (
          !keepSelection &&
          normalizedLocations.length > 0 &&
          !selectedPickupId
        ) {
          setSelectedPickupId(
            Number(normalizedLocations[0].id)
          );
        }

        /*
         * Keep selected route synchronized after refresh.
         */
        if (selectedRoute?.id) {
          const refreshed = Object.values(
            normalizedMap
          ).find(
            (item) =>
              Number(item?.id) ===
              Number(selectedRoute.id)
          );

          if (refreshed) {
            setSelectedRoute(refreshed);
          }
        }
      } catch (error) {
        message.error(
          apiErrorMessage(
            error,
            "Could not load branch pricing matrix."
          )
        );
      } finally {
        setLoading(false);
      }
    },
    [selectedPickupId, selectedRoute]
  );

  useEffect(() => {
    loadMatrix();

    getPricingSettings()
      .then((response) => {
        setActivePricingSettings(
          response?.active ?? null
        );
      })
      .catch(() => {});
  }, [loadMatrix]);

  /*
  |--------------------------------------------------------------------------
  | STATS
  |--------------------------------------------------------------------------
  */

  const stats = useMemo(() => {
    const rates = Object.values(rateMap);

    const total = rates.length;

    const active = rates.filter(
      (rate) => toBoolean(rate?.is_active)
    ).length;

    const n = locations.length;

    const coverage =
      n > 0
        ? Math.round((total / (n * n)) * 100)
        : 0;

    return {
      total,
      active,
      coverage,
      locationCount: n,
    };
  }, [rateMap, locations]);

  /*
  |--------------------------------------------------------------------------
  | DESTINATION ROWS
  |--------------------------------------------------------------------------
  */

  const destinationRows = useMemo(() => {
    if (!selectedPickupId) {
      return [];
    }

    return locations.map((location) => ({
      location,
      rate:
        rateMap[
          `${selectedPickupId}:${Number(location.id)}`
        ] ?? null,
    }));
  }, [selectedPickupId, locations, rateMap]);

  /*
  |--------------------------------------------------------------------------
  | FILTER DESTINATIONS
  |--------------------------------------------------------------------------
  */

  const filteredDestinations = useMemo(() => {
    let rows = destinationRows;

    const q = search.trim().toLowerCase();

    if (q) {
      rows = rows.filter(({ location }) =>
        location.name
          ?.toLowerCase()
          .includes(q) ||
        location.code
          ?.toLowerCase()
          .includes(q)
      );
    }

    if (destTab === "active") {
      rows = rows.filter(
        ({ rate }) =>
          rate &&
          toBoolean(rate.is_active)
      );
    }

    if (destTab === "missing") {
      rows = rows.filter(({ rate }) => !rate);
    }

    return rows;
  }, [
    destinationRows,
    search,
    destTab,
  ]);

  /*
  |--------------------------------------------------------------------------
  | FILTER BRANCHES
  |--------------------------------------------------------------------------
  */

  const filteredBranches = useMemo(() => {
    let list = locations;

    const q = branchSearch
      .trim()
      .toLowerCase();

    if (q) {
      list = list.filter(
        (location) =>
          location.name
            ?.toLowerCase()
            .includes(q) ||
          location.code
            ?.toLowerCase()
            .includes(q)
      );
    }

    if (branchTab === "complete") {
      list = list.filter((location) => {
        const locationId = Number(location.id);

        const count = Object.keys(rateMap).filter(
          (key) =>
            key.startsWith(`${locationId}:`)
        ).length;

        return count >= locations.length;
      });
    }

    if (branchTab === "missing") {
      list = list.filter((location) => {
        const locationId = Number(location.id);

        const count = Object.keys(rateMap).filter(
          (key) =>
            key.startsWith(`${locationId}:`)
        ).length;

        return count < locations.length;
      });
    }

    return list;
  }, [
    locations,
    rateMap,
    branchSearch,
    branchTab,
  ]);

  /*
  |--------------------------------------------------------------------------
  | PAGED BRANCHES
  |--------------------------------------------------------------------------
  */

  const pagedBranches = useMemo(() => {
    const start =
      (branchPage - 1) *
      BRANCH_PAGE_SIZE;

    return filteredBranches.slice(
      start,
      start + BRANCH_PAGE_SIZE
    );
  }, [
    filteredBranches,
    branchPage,
  ]);

  /*
  |--------------------------------------------------------------------------
  | SELECTED PICKUP
  |--------------------------------------------------------------------------
  */

  const selectedPickupLocation = useMemo(
    () =>
      locations.find(
        (location) =>
          Number(location.id) ===
          Number(selectedPickupId)
      ) ?? null,
    [locations, selectedPickupId]
  );

  const missingCount = destinationRows.filter(
    ({ rate }) => !rate
  ).length;

  /*
  |--------------------------------------------------------------------------
  | INLINE ADD
  |--------------------------------------------------------------------------
  */

  const startAdd = (locationId) => {
    setInlineEdit(null);
    setInlineAdd(Number(locationId));

    setInlineForm({
      ...emptyForm(),
    });
  };

  /*
  |--------------------------------------------------------------------------
  | CANCEL INLINE
  |--------------------------------------------------------------------------
  */

  const cancelInline = () => {
    setInlineAdd(null);
    setInlineEdit(null);
    setInlineForm({
      ...emptyForm(),
    });
  };

  /*
  |--------------------------------------------------------------------------
  | PATCH RATE MAP
  |--------------------------------------------------------------------------
  */

  const patchRateMap = useCallback(
    (pickupId, deliveryId, rate) => {
      const pickup = Number(pickupId);
      const delivery = Number(deliveryId);

      const key = `${pickup}:${delivery}`;

      const normalized = normalizeLocalRate(rate);

      setRateMap((previous) => ({
        ...previous,
        [key]: normalized,
      }));

      setSelectedRoute((previous) => {
        if (
          previous &&
          Number(previous.id) ===
            Number(normalized.id)
        ) {
          return normalized;
        }

        return previous;
      });
    },
    []
  );

  /*
  |--------------------------------------------------------------------------
  | REMOVE RATE FROM MAP
  |--------------------------------------------------------------------------
  */

  const dropFromRateMap = useCallback(
    (pickupId, deliveryId, rateId) => {
      const key = `${Number(pickupId)}:${Number(
        deliveryId
      )}`;

      setRateMap((previous) => {
        const next = {
          ...previous,
        };

        delete next[key];

        return next;
      });

      setSelectedRoute((previous) =>
        previous &&
        Number(previous.id) ===
          Number(rateId)
          ? null
          : previous
      );
    },
    []
  );

  /*
  |--------------------------------------------------------------------------
  | INLINE ADD SAVE
  |--------------------------------------------------------------------------
  */

  const saveAdd = async (deliveryLocationId) => {
    if (
      inlineForm.base_rate ===
        null ||
      inlineForm.base_rate ===
        undefined ||
      Number(inlineForm.base_rate) < 0
    ) {
      message.warning(
        "Enter a valid base rate."
      );
      return;
    }

    const pickupId = Number(
      selectedPickupId
    );

    const deliveryId = Number(
      deliveryLocationId
    );

    const payload = {
      pickup_coverage_location_id:
        pickupId,

      delivery_coverage_location_id:
        deliveryId,

      base_rate: Number(
        inlineForm.base_rate ?? 0
      ),

      is_active: Boolean(
        inlineForm.is_active
      ),

      express_enabled: Boolean(
        inlineForm.express_enabled
      ),

      same_day_enabled: Boolean(
        inlineForm.same_day_enabled
      ),
    };

    try {
      setInlineSaving(true);

      const response =
        await createBranchRouteRate(
          payload
        );

      const apiObject =
        extractApiObject(response);

      /*
       * Some APIs return:
       *
       * { forward_id: 123 }
       *
       * instead of the complete rate object.
       */
      const createdRate = {
        ...(apiObject || {}),

        id:
          apiObject?.id ??
          response?.forward_id ??
          response?.data?.forward_id ??
          response?.forwardId ??
          null,

        pickup_coverage_location_id:
          pickupId,

        delivery_coverage_location_id:
          deliveryId,

        /*
         * These are deliberately placed AFTER the API response
         * so an incomplete API response cannot erase them.
         */
        base_rate: Number(
          payload.base_rate
        ),

        is_active:
          payload.is_active,

        express_enabled:
          payload.express_enabled,

        same_day_enabled:
          payload.same_day_enabled,
      };

      patchRateMap(
        pickupId,
        deliveryId,
        createdRate
      );

      message.success(
        "Rate added successfully."
      );

      cancelInline();
    } catch (error) {
      message.error(
        apiErrorMessage(
          error,
          "Could not save rate."
        )
      );
    } finally {
      setInlineSaving(false);
    }
  };

  /*
  |--------------------------------------------------------------------------
  | START INLINE EDIT
  |--------------------------------------------------------------------------
  */

  const startEdit = (rate) => {
    setInlineAdd(null);

    setInlineEdit(rate.id);

    setInlineForm({
      base_rate: Number(
        rate.base_rate ?? 0
      ),

      is_active: toBoolean(
        rate.is_active,
        true
      ),

      express_enabled: toBoolean(
        rate.express_enabled,
        true
      ),

      same_day_enabled: toBoolean(
        rate.same_day_enabled,
        true
      ),
    });
  };

  /*
  |--------------------------------------------------------------------------
  | INLINE EDIT SAVE
  |--------------------------------------------------------------------------
  |
  | THIS FIXES THE TOGGLE PROBLEM.
  |
  | We build the submitted values once, then merge those values BACK over
  | the API response. This prevents a partial/stale backend response from
  | immediately reverting the Switch state in the UI.
  |
  */

  const saveEdit = async (rate) => {
    if (!rate?.id) {
      message.error(
        "Invalid rate selected."
      );
      return;
    }

    const pickupId =
      getPickupId(rate);

    const deliveryId =
      getDeliveryId(rate);

    if (!pickupId || !deliveryId) {
      message.error(
        "Pickup or delivery zone is missing."
      );
      return;
    }

    /*
     * Build a completely normalized payload.
     */
    const payload = {
      pickup_coverage_location_id:
        pickupId,

      delivery_coverage_location_id:
        deliveryId,

      base_rate: Number(
        inlineForm.base_rate ?? 0
      ),

      is_active: Boolean(
        inlineForm.is_active
      ),

      express_enabled: Boolean(
        inlineForm.express_enabled
      ),

      same_day_enabled: Boolean(
        inlineForm.same_day_enabled
      ),
    };

    try {
      setInlineSaving(true);

      const response =
        await updateBranchRouteRate(
          rate.id,
          payload
        );

      const apiObject =
        extractApiObject(response);

      /*
       * CRITICAL:
       *
       * The submitted values are placed AFTER the API response.
       *
       * Therefore if Laravel returns only:
       *
       * {
       *   id: 10,
       *   base_rate: 100
       * }
       *
       * the local state still retains:
       *
       * express_enabled
       * same_day_enabled
       * is_active
       *
       * from the values we just saved.
       */
      const updatedRate = normalizeLocalRate(
        {
          ...(apiObject || {}),

          ...rate,

          id: rate.id,

          pickup_coverage_location_id:
            pickupId,

          delivery_coverage_location_id:
            deliveryId,

          base_rate:
            payload.base_rate,

          is_active:
            payload.is_active,

          express_enabled:
            payload.express_enabled,

          same_day_enabled:
            payload.same_day_enabled,
        },
        {
          ...rate,
          ...payload,
        }
      );

      /*
       * Update the exact matrix cell immediately.
       */
      patchRateMap(
        pickupId,
        deliveryId,
        updatedRate
      );

      /*
       * Keep selected route detail synchronized.
       */
      setSelectedRoute(
        (previous) => {
          if (
            previous &&
            Number(previous.id) ===
              Number(rate.id)
          ) {
            return updatedRate;
          }

          return previous;
        }
      );

      message.success(
        "Rate updated successfully."
      );

      cancelInline();
    } catch (error) {
      message.error(
        apiErrorMessage(
          error,
          "Could not update rate."
        )
      );
    } finally {
      setInlineSaving(false);
    }
  };

  /*
  |--------------------------------------------------------------------------
  | ACTIVE STATUS TOGGLE
  |--------------------------------------------------------------------------
  */

  const toggleStatus = async (rate) => {
    if (!rate?.id) {
      message.error(
        "Invalid rate selected."
      );
      return;
    }

    const pickupId =
      getPickupId(rate);

    const deliveryId =
      getDeliveryId(rate);

    const nextActive =
      !toBoolean(rate.is_active);

    try {
      await updateBranchRouteRateStatus(
        rate.id,
        nextActive
      );

      /*
       * Preserve every other value.
       */
      const updatedRate = normalizeLocalRate(
        {
          ...rate,
          is_active: nextActive,
        },
        rate
      );

      patchRateMap(
        pickupId,
        deliveryId,
        updatedRate
      );

      setSelectedRoute(
        (previous) => {
          if (
            previous &&
            Number(previous.id) ===
              Number(rate.id)
          ) {
            return updatedRate;
          }

          return previous;
        }
      );

      message.success(
        `Rate ${
          nextActive
            ? "enabled"
            : "disabled"
        }.`
      );
    } catch (error) {
      message.error(
        apiErrorMessage(
          error,
          "Could not update status."
        )
      );
    }
  };

  /*
  |--------------------------------------------------------------------------
  | CREATE REVERSE RATE
  |--------------------------------------------------------------------------
  |
  | FIX:
  |
  | Reverse rate is explicitly given the same commercial base rate.
  |
  | Example:
  |
  | KTM -> Kavre = NPR 89
  |
  | Reverse:
  |
  | Kavre -> KTM = NPR 89
  |
  */

  const createReverse = async (rate) => {
    if (!rate?.id) {
      message.error(
        "Invalid rate selected."
      );
      return;
    }

    const originalPickupId =
      getPickupId(rate);

    const originalDeliveryId =
      getDeliveryId(rate);

    if (
      !originalPickupId ||
      !originalDeliveryId
    ) {
      message.error(
        "Pickup or delivery zone is missing."
      );
      return;
    }

    if (
      originalPickupId ===
      originalDeliveryId
    ) {
      message.warning(
        "A same-branch route does not need a reverse rate."
      );
      return;
    }

    const reversePickupId =
      originalDeliveryId;

    const reverseDeliveryId =
      originalPickupId;

    /*
     * IMPORTANT:
     * The reverse price comes from the original commercial price.
     */
    const reverseBaseRate = Number(
      rate.base_rate ?? 0
    );

    const reversePayload = {
      pickup_coverage_location_id:
        reversePickupId,

      delivery_coverage_location_id:
        reverseDeliveryId,

      base_rate:
        reverseBaseRate,

      is_active: toBoolean(
        rate.is_active,
        true
      ),

      express_enabled: toBoolean(
        rate.express_enabled,
        true
      ),

      same_day_enabled: toBoolean(
        rate.same_day_enabled,
        true
      ),
    };

    try {
      setInlineSaving(true);

      const response =
        await createBranchRouteRate(
          reversePayload
        );

      const apiObject =
        extractApiObject(response);

      /*
       * IMPORTANT:
       *
       * API response is spread FIRST.
       *
       * Our actual reverse pricing values are spread AFTER it.
       *
       * This prevents an API response such as:
       *
       * { id: 22, base_rate: null }
       *
       * from destroying the reverse price.
       */
      const reverseRate = normalizeLocalRate(
        {
          ...(apiObject || {}),

          id:
            apiObject?.id ??
            response?.forward_id ??
            response?.data?.forward_id ??
            response?.id ??
            null,

          pickup_coverage_location_id:
            reversePickupId,

          delivery_coverage_location_id:
            reverseDeliveryId,

          /*
           * Commercial pricing comes from the original rate.
           */
          base_rate:
            reverseBaseRate,

          is_active:
            reversePayload.is_active,

          express_enabled:
            reversePayload.express_enabled,

          same_day_enabled:
            reversePayload.same_day_enabled,
        },
        {
          pickup_coverage_location_id:
            reversePickupId,

          delivery_coverage_location_id:
            reverseDeliveryId,

          base_rate:
            reverseBaseRate,

          is_active:
            reversePayload.is_active,

          express_enabled:
            reversePayload.express_enabled,

          same_day_enabled:
            reversePayload.same_day_enabled,
        }
      );

      /*
       * Make sure we have an ID.
       */
      if (!reverseRate.id) {
        /*
         * The backend may have created it but not returned the ID.
         * Refresh the matrix so the reverse route becomes visible.
         */
        await loadMatrix(true);

        message.success(
          "Reverse rate created successfully."
        );

        return;
      }

      patchRateMap(
        reversePickupId,
        reverseDeliveryId,
        reverseRate
      );

      message.success(
        `Reverse rate created: ${formatMoney(
          reverseBaseRate
        )}`
      );
    } catch (error) {
      message.error(
        apiErrorMessage(
          error,
          "Could not create reverse rate."
        )
      );
    } finally {
      setInlineSaving(false);
    }
  };

  /*
  |--------------------------------------------------------------------------
  | DELETE
  |--------------------------------------------------------------------------
  */

  const removeRate = async (rate) => {
    if (!rate?.id) {
      message.error(
        "Invalid rate selected."
      );
      return;
    }

    try {
      await deleteBranchRouteRate(
        rate.id
      );

      dropFromRateMap(
        getPickupId(rate),
        getDeliveryId(rate),
        rate.id
      );

      message.success(
        "Rate deleted successfully."
      );
    } catch (error) {
      message.error(
        apiErrorMessage(
          error,
          "Could not delete rate."
        )
      );
    }
  };

  /*
  |--------------------------------------------------------------------------
  | INLINE FORM FIELDS
  |--------------------------------------------------------------------------
  */

  const InlineFormFields = ({
    onSave,
    onCancel,
    saving,
  }) => (
    <div
      style={{
        display: "flex",
        alignItems: "center",
        gap: 10,
        flexWrap: "wrap",
      }}
    >
      {/* BASE RATE */}

      <div>
        <Text
          type="secondary"
          style={{
            fontSize: 10,
            display: "block",
            marginBottom: 3,
          }}
        >
          Base Rate (NPR)
        </Text>

        <InputNumber
          min={0}
          step={10}
          value={inlineForm.base_rate}
          onChange={(value) =>
            setInlineForm((previous) => ({
              ...previous,
              base_rate:
                value === null ||
                value === undefined
                  ? 0
                  : Number(value),
            }))
          }
          style={{ width: 120 }}
          size="small"
          disabled={saving}
          autoFocus
        />
      </div>

      {/* EXPRESS */}

      <div>
        <Text
          type="secondary"
          style={{
            fontSize: 10,
            display: "block",
            marginBottom: 3,
          }}
        >
          Express
        </Text>

        <Switch
          size="small"
          checked={Boolean(
            inlineForm.express_enabled
          )}
          onChange={(checked) =>
            setInlineForm((previous) => ({
              ...previous,
              express_enabled:
                Boolean(checked),
            }))
          }
          checkedChildren="On"
          unCheckedChildren="Off"
          disabled={saving}
        />
      </div>

      {/* SAME DAY */}

      <div>
        <Text
          type="secondary"
          style={{
            fontSize: 10,
            display: "block",
            marginBottom: 3,
          }}
        >
          Same Day
        </Text>

        <Switch
          size="small"
          checked={Boolean(
            inlineForm.same_day_enabled
          )}
          onChange={(checked) =>
            setInlineForm((previous) => ({
              ...previous,
              same_day_enabled:
                Boolean(checked),
            }))
          }
          checkedChildren="On"
          unCheckedChildren="Off"
          disabled={saving}
        />
      </div>

      {/* ACTIVE */}

      <div>
        <Text
          type="secondary"
          style={{
            fontSize: 10,
            display: "block",
            marginBottom: 3,
          }}
        >
          Active
        </Text>

        <Switch
          size="small"
          checked={Boolean(
            inlineForm.is_active
          )}
          onChange={(checked) =>
            setInlineForm((previous) => ({
              ...previous,
              is_active:
                Boolean(checked),
            }))
          }
          checkedChildren="Yes"
          unCheckedChildren="No"
          disabled={saving}
        />
      </div>

      {/* ACTIONS */}

      <Space
        size={4}
        style={{ marginTop: 14 }}
      >
        <Button
          type="primary"
          size="small"
          icon={<SaveOutlined />}
          loading={saving}
          onClick={onSave}
        >
          Save
        </Button>

        <Button
          size="small"
          icon={<CloseOutlined />}
          onClick={onCancel}
          disabled={saving}
        >
          Cancel
        </Button>
      </Space>
    </div>
  );

  /*
  |--------------------------------------------------------------------------
  | DESTINATION ROW
  |--------------------------------------------------------------------------
  */

  const DestinationRow = ({
    location,
    rate,
  }) => {
    const locId = Number(
      location.id
    );

    const isSelf =
      locId ===
      Number(selectedPickupId);

    const hasRate = Boolean(rate);

    const isSelected =
      hasRate &&
      selectedRoute &&
      Number(selectedRoute.id) ===
        Number(rate?.id);

    const isEditing =
      hasRate &&
      Number(inlineEdit) ===
        Number(rate?.id);

    const isAdding =
      !hasRate &&
      Number(inlineAdd) === locId;

    const reverseKey = `${locId}:${Number(
      selectedPickupId
    )}`;

    const hasReverseRate =
      hasRate &&
      Boolean(rateMap[reverseKey]);

    return (
      <div
        style={{
          marginBottom: 6,
          borderRadius: 8,

          border: isSelected
            ? "1px solid #91caff"
            : isAdding || isEditing
              ? "1px solid #b7eb8f"
              : hasRate
                ? "1px solid #edf0f3"
                : "1px dashed #ffd666",

          borderLeft: isSelected
            ? "3px solid #1677ff"
            : isAdding || isEditing
              ? "3px solid #52c41a"
              : hasRate
                ? "1px solid #edf0f3"
                : "3px solid #faad14",

          background: isSelected
            ? "#f0f7ff"
            : isAdding || isEditing
              ? "#f6ffed"
              : hasRate
                ? "#fff"
                : "#fffbe6",

          overflow: "hidden",

          transition:
            "all .15s ease",
        }}
      >
        {/* MAIN ROW */}

        <div
          onClick={() => {
            if (
              hasRate &&
              !isEditing
            ) {
              setSelectedRoute(rate);
            }
          }}
          style={{
            display: "grid",
            gridTemplateColumns:
              "minmax(140px, 1.2fr) 1fr 80px 80px 80px 170px",
            gap: 10,
            alignItems: "center",
            minHeight: 58,
            padding: "8px 14px",
            cursor:
              hasRate && !isEditing
                ? "pointer"
                : "default",
          }}
        >
          {/* DESTINATION */}

          <div
            style={{
              minWidth: 0,
            }}
          >
            <div
              style={{
                display: "flex",
                alignItems: "center",
                gap: 5,
                flexWrap: "wrap",
              }}
            >
              <Text
                strong
                style={{
                  fontSize: 13,
                }}
              >
                {location.name}
              </Text>

              {isSelf && (
                <Tag
                  color="blue"
                  style={{
                    margin: 0,
                    fontSize: 10,
                  }}
                >
                  Local
                </Tag>
              )}
            </div>

            <Text
              type="secondary"
              style={{
                fontSize: 11,
              }}
            >
              {location.code}
            </Text>
          </div>

          {/* BASE RATE */}

          <div>
            {hasRate ? (
              <>
                <Text
                  type="secondary"
                  style={{
                    fontSize: 10,
                  }}
                >
                  Standard
                </Text>

                <div
                  style={{
                    fontWeight: 600,
                    fontSize: 13,
                    marginTop: 1,
                  }}
                >
                  {formatMoney(
                    rate.base_rate
                  )}
                </div>
              </>
            ) : (
              <Text
                type="secondary"
                style={{
                  fontSize: 12,
                }}
              >
                —
              </Text>
            )}
          </div>

          {/* EXPRESS */}

          <div>
            {hasRate && (
              <Tag
                color={
                  rate.express_enabled ===
                  false
                    ? "default"
                    : "orange"
                }
                style={{
                  margin: 0,
                  fontSize: 10,
                }}
              >
                {rate.express_enabled ===
                false
                  ? "Off"
                  : "Express"}
              </Tag>
            )}
          </div>

          {/* SAME DAY */}

          <div>
            {hasRate && (
              <Tag
                color={
                  rate.same_day_enabled ===
                  false
                    ? "default"
                    : "magenta"
                }
                style={{
                  margin: 0,
                  fontSize: 10,
                }}
              >
                {rate.same_day_enabled ===
                false
                  ? "Off"
                  : "Same Day"}
              </Tag>
            )}
          </div>

          {/* STATUS */}

          <div>
            {hasRate ? (
              <StatusTag
                active={toBoolean(
                  rate.is_active
                )}
              />
            ) : (
              <Tag
                icon={
                  <WarningOutlined />
                }
                color="warning"
                style={{
                  margin: 0,
                }}
              >
                Missing
              </Tag>
            )}
          </div>

          {/* ACTIONS */}

          <Space
            size={4}
            onClick={(event) =>
              event.stopPropagation()
            }
          >
            {hasRate ? (
              isEditing ? null : (
                <>
                  <Tooltip title="Edit inline">
                    <Button
                      size="small"
                      icon={
                        <EditOutlined />
                      }
                      onClick={() =>
                        startEdit(rate)
                      }
                    />
                  </Tooltip>

                  <Tooltip
                    title={
                      isSelf
                        ? "Local route"
                        : hasReverseRate
                          ? "Reverse rate already exists"
                          : "Create reverse rate"
                    }
                  >
                    <Button
                      size="small"
                      disabled={
                        isSelf ||
                        hasReverseRate
                      }
                      icon={
                        <SwapOutlined />
                      }
                      onClick={() =>
                        createReverse(
                          rate
                        )
                      }
                    />
                  </Tooltip>

                  <Button
                    size="small"
                    onClick={() =>
                      toggleStatus(
                        rate
                      )
                    }
                  >
                    {toBoolean(
                      rate.is_active
                    )
                      ? "Disable"
                      : "Enable"}
                  </Button>

                  <Popconfirm
                    title="Delete this rate?"
                    description="This cannot be undone."
                    okText="Delete"
                    cancelText="Cancel"
                    okButtonProps={{
                      danger: true,
                    }}
                    onConfirm={() =>
                      removeRate(rate)
                    }
                  >
                    <Button
                      danger
                      size="small"
                      icon={
                        <DeleteOutlined />
                      }
                    />
                  </Popconfirm>
                </>
              )
            ) : isAdding ? null : (
              <Button
                type="primary"
                size="small"
                icon={
                  <PlusOutlined />
                }
                onClick={() =>
                  startAdd(locId)
                }
              >
                Add Rate
              </Button>
            )}
          </Space>
        </div>

        {/* INLINE EDIT */}

        {isEditing && (
          <div
            style={{
              padding:
                "10px 14px 12px",
              borderTop:
                "1px solid #d9f7be",
              background: "#f6ffed",
            }}
          >
            <InlineFormFields
              onSave={() =>
                saveEdit(rate)
              }
              onCancel={
                cancelInline
              }
              saving={
                inlineSaving
              }
            />
          </div>
        )}

        {/* INLINE ADD */}

        {isAdding && (
          <div
            style={{
              padding:
                "10px 14px 12px",
              borderTop:
                "1px solid #d9f7be",
              background: "#f6ffed",
            }}
          >
            <InlineFormFields
              onSave={() =>
                saveAdd(locId)
              }
              onCancel={
                cancelInline
              }
              saving={
                inlineSaving
              }
            />
          </div>
        )}
      </div>
    );
  };

  /*
  |--------------------------------------------------------------------------
  | MAP NODES
  |--------------------------------------------------------------------------
  */

  const selectedRouteNodes =
    selectedRoute
      ? [
          locations.find(
            (location) =>
              Number(location.id) ===
              getPickupId(
                selectedRoute
              )
          ),

          locations.find(
            (location) =>
              Number(location.id) ===
              getDeliveryId(
                selectedRoute
              )
          ),
        ].filter(Boolean)
      : [];

  /*
  |--------------------------------------------------------------------------
  | RENDER
  |--------------------------------------------------------------------------
  */

  return (
    <div
      style={{
        width: "100%",
        padding:
          "20px 22px 32px",
        background:
          "#f5f7fa",
        minHeight:
          "100vh",
      }}
    >
      {/* HEADER */}

      <Card
        bordered={false}
        style={{
          borderRadius: 12,
          marginBottom: 16,
        }}
        styles={{
          body: {
            padding:
              "18px 20px",
          },
        }}
      >
        <Row
          justify="space-between"
          align="middle"
        >
          <Col>
            <Title
              level={3}
              style={{
                margin: 0,
              }}
            >
              Branch Pricing
            </Title>

            <Text type="secondary">
              Select a branch to view
              and manage its delivery
              rates.
            </Text>
          </Col>

          <Col>
            <Button
              icon={
                <ReloadOutlined />
              }
              loading={loading}
              onClick={() =>
                loadMatrix(true)
              }
            >
              Refresh
            </Button>
          </Col>
        </Row>
      </Card>

      {/* STATS */}

      <Row
        gutter={[12, 12]}
        style={{
          marginBottom: 16,
        }}
      >
        {[
          {
            title: "Branches",
            value:
              stats.locationCount,
            prefix: (
              <EnvironmentOutlined />
            ),
          },
          {
            title:
              "Total Routes",
            value:
              stats.total,
          },
          {
            title:
              "Active Routes",
            value:
              stats.active,
            suffix:
              `/ ${stats.total}`,
            valueStyle: {
              color:
                "#52c41a",
            },
          },
          {
            title:
              "Matrix Coverage",
            value:
              stats.coverage,
            suffix: "%",
            valueStyle: {
              color:
                stats.coverage ===
                100
                  ? "#52c41a"
                  : "#faad14",
            },
          },
        ].map((stat) => (
          <Col
            key={stat.title}
            xs={24}
            sm={12}
            lg={6}
          >
            <Card
              bordered={false}
              style={{
                borderRadius: 10,
              }}
            >
              <Statistic
                {...stat}
              />
            </Card>
          </Col>
        ))}
      </Row>

      {/* MAIN */}

      <Row
        gutter={[16, 16]}
        align="top"
      >
        {/* LEFT */}

        <Col
          xs={24}
          xl={5}
        >
          <Card
            bordered={false}
            style={{
              borderRadius: 12,
            }}
            styles={{
              body: {
                padding: 0,
              },
            }}
            title={
              <Space>
                <EnvironmentOutlined />

                <Text strong>
                  Branches
                </Text>

                <Tag color="blue">
                  {locations.length}
                </Tag>
              </Space>
            }
          >
            <div
              style={{
                padding:
                  "10px 12px 0",
              }}
            >
              <Input
                allowClear
                size="small"
                placeholder="Search branch..."
                value={
                  branchSearch
                }
                onChange={(event) => {
                  setBranchSearch(
                    event.target.value
                  );
                  setBranchPage(1);
                }}
              />
            </div>

            <Tabs
              size="small"
              activeKey={
                branchTab
              }
              onChange={(key) => {
                setBranchTab(key);
                setBranchPage(1);
              }}
              style={{
                padding:
                  "0 12px",
              }}
              items={[
                {
                  key: "all",
                  label: `All (${locations.length})`,
                },
                {
                  key: "missing",
                  label: (
                    <span
                      style={{
                        color:
                          "#faad14",
                      }}
                    >
                      Missing
                    </span>
                  ),
                },
                {
                  key: "complete",
                  label: (
                    <span
                      style={{
                        color:
                          "#52c41a",
                      }}
                    >
                      Complete
                    </span>
                  ),
                },
              ]}
            />

            {loading ? (
              <div
                style={{
                  padding: 40,
                  textAlign:
                    "center",
                }}
              >
                Loading...
              </div>
            ) : !filteredBranches.length ? (
              <div
                style={{
                  padding: 40,
                }}
              >
                <Empty description="No branches" />
              </div>
            ) : (
              <>
                <div
                  style={{
                    padding:
                      "0 0 6px",
                  }}
                >
                  {pagedBranches.map(
                    (location) => {
                      const locId =
                        Number(
                          location.id
                        );

                      const isActive =
                        locId ===
                        Number(
                          selectedPickupId
                        );

                      const ratesCount =
                        Object.keys(
                          rateMap
                        ).filter(
                          (key) =>
                            key.startsWith(
                              `${locId}:`
                            )
                        ).length;

                      const missing =
                        locations.length -
                        ratesCount;

                      return (
                        <div
                          key={
                            location.id
                          }
                          onClick={() => {
                            setSelectedPickupId(
                              locId
                            );

                            setSelectedRoute(
                              null
                            );

                            setSearch(
                              ""
                            );

                            cancelInline();
                          }}
                          style={{
                            display:
                              "flex",
                            alignItems:
                              "center",
                            justifyContent:
                              "space-between",
                            padding:
                              "10px 16px",
                            cursor:
                              "pointer",
                            background:
                              isActive
                                ? "#e6f4ff"
                                : "transparent",
                            borderLeft:
                              isActive
                                ? "3px solid #1677ff"
                                : "3px solid transparent",
                            transition:
                              "all .15s",
                          }}
                        >
                          <div>
                            <Text
                              strong
                              style={{
                                fontSize: 13,
                              }}
                            >
                              {
                                location.name
                              }
                            </Text>

                            <Text
                              type="secondary"
                              style={{
                                display:
                                  "block",
                                fontSize: 11,
                              }}
                            >
                              {
                                location.code
                              }
                            </Text>
                          </div>

                          <div
                            style={{
                              textAlign:
                                "right",
                            }}
                          >
                            <div
                              style={{
                                fontSize: 11,
                                color:
                                  "#52c41a",
                              }}
                            >
                              {
                                ratesCount
                              }{" "}
                              routes
                            </div>

                            {missing >
                              0 && (
                              <Badge
                                count={
                                  missing
                                }
                                size="small"
                                color="#faad14"
                              />
                            )}
                          </div>
                        </div>
                      );
                    }
                  )}
                </div>

                {filteredBranches.length >
                  BRANCH_PAGE_SIZE && (
                  <div
                    style={{
                      padding:
                        "8px 12px 12px",
                      textAlign:
                        "center",
                      borderTop:
                        "1px solid #f0f0f0",
                    }}
                  >
                    <Pagination
                      simple
                      size="small"
                      current={
                        branchPage
                      }
                      pageSize={
                        BRANCH_PAGE_SIZE
                      }
                      total={
                        filteredBranches.length
                      }
                      onChange={(page) =>
                        setBranchPage(
                          page
                        )
                      }
                    />
                  </div>
                )}
              </>
            )}
          </Card>
        </Col>

        {/* MIDDLE */}

        <Col
          xs={24}
          xl={12}
        >
          <Card
            bordered={false}
            style={{
              borderRadius: 12,
            }}
            styles={{
              body: {
                padding: 0,
              },
            }}
            title={
              selectedPickupLocation ? (
                <Space wrap>
                  <Text strong>
                    From:{" "}
                    {
                      selectedPickupLocation.name
                    }
                  </Text>

                  <Tag color="blue">
                    {
                      selectedPickupLocation.code
                    }
                  </Tag>

                  {missingCount >
                    0 && (
                    <Tag
                      color="warning"
                      icon={
                        <WarningOutlined />
                      }
                    >
                      {
                        missingCount
                      }{" "}
                      missing
                    </Tag>
                  )}
                </Space>
              ) : (
                <Text type="secondary">
                  Select a branch
                </Text>
              )
            }
          >
            {!selectedPickupId ? (
              <div
                style={{
                  padding: 60,
                }}
              >
                <Empty description="Select a branch on the left" />
              </div>
            ) : (
              <>
                <div
                  style={{
                    padding:
                      "10px 14px 0",
                  }}
                >
                  <Input
                    allowClear
                    placeholder="Search destination..."
                    value={search}
                    onChange={(event) =>
                      setSearch(
                        event.target
                          .value
                      )
                    }
                    size="small"
                  />
                </div>

                <Tabs
                  size="small"
                  activeKey={
                    destTab
                  }
                  onChange={
                    setDestTab
                  }
                  style={{
                    padding:
                      "0 14px",
                  }}
                  items={[
                    {
                      key: "all",
                      label: `All (${destinationRows.length})`,
                    },
                    {
                      key: "active",
                      label: (
                        <span
                          style={{
                            color:
                              "#52c41a",
                          }}
                        >
                          Active (
                          {
                            destinationRows.filter(
                              ({
                                rate,
                              }) =>
                                rate &&
                                toBoolean(
                                  rate.is_active
                                )
                            ).length
                          }
                          )
                        </span>
                      ),
                    },
                    {
                      key: "missing",
                      label: (
                        <span
                          style={{
                            color:
                              "#faad14",
                          }}
                        >
                          Missing (
                          {
                            destinationRows.filter(
                              ({
                                rate,
                              }) =>
                                !rate
                            ).length
                          }
                          )
                        </span>
                      ),
                    },
                  ]}
                />

                <div
                  style={{
                    padding:
                      "4px 14px 14px",
                  }}
                >
                  {filteredDestinations.map(
                    ({
                      location,
                      rate,
                    }) => (
                      <DestinationRow
                        key={
                          location.id
                        }
                        location={
                          location
                        }
                        rate={rate}
                      />
                    )
                  )}
                </div>
              </>
            )}
          </Card>
        </Col>

        {/* RIGHT */}

        <Col
          xs={24}
          xl={7}
          style={{
            alignSelf:
              "flex-start",
          }}
        >
          <div
            style={{
              position:
                "sticky",
              top: 16,
            }}
          >
            <Card
              bordered={false}
              style={{
                borderRadius: 12,
                overflow:
                  "hidden",
              }}
              styles={{
                body: {
                  padding: 0,
                },
              }}
              title={
                <Space>
                  <EnvironmentOutlined
                    style={{
                      color:
                        "#1677ff",
                    }}
                  />

                  <Text strong>
                    Route Detail
                  </Text>
                </Space>
              }
            >
              {selectedRoute ? (
                <>
                  <div
                    style={{
                      padding:
                        "12px 16px",
                      background:
                        "#f7fbff",
                      borderBottom:
                        "1px solid #e6f4ff",
                    }}
                  >
                    <Text
                      type="secondary"
                      style={{
                        fontSize: 10,
                        textTransform:
                          "uppercase",
                        letterSpacing:
                          ".08em",
                      }}
                    >
                      Delivery Route
                    </Text>

                    <div
                      style={{
                        display:
                          "flex",
                        alignItems:
                          "center",
                        gap: 8,
                        flexWrap:
                          "wrap",
                        marginTop: 6,
                      }}
                    >
                      <Text strong>
                        {
                          locations.find(
                            (location) =>
                              Number(
                                location.id
                              ) ===
                              getPickupId(
                                selectedRoute
                              )
                          )?.name
                        }
                      </Text>

                      <span
                        style={{
                          color:
                            "#1677ff",
                          fontSize: 18,
                        }}
                      >
                        →
                      </span>

                      <Text strong>
                        {
                          locations.find(
                            (location) =>
                              Number(
                                location.id
                              ) ===
                              getDeliveryId(
                                selectedRoute
                              )
                          )?.name
                        }
                      </Text>
                    </div>
                  </div>

                  <div
                    style={{
                      padding: 12,
                    }}
                  >
                    <RouteMapS
                      nodes={
                        selectedRouteNodes
                      }
                      height={260}
                      selectedLabel="Route"
                    />
                  </div>

                  <div
                    style={{
                      padding:
                        "0 12px 14px",
                    }}
                  >
                    <Row
                      gutter={8}
                      style={{
                        marginBottom: 10,
                      }}
                    >
                      <Col span={12}>
                        <div
                          style={{
                            padding: 10,
                            border:
                              "1px solid #f0f0f0",
                            borderRadius: 8,
                          }}
                        >
                          <Text
                            type="secondary"
                            style={{
                              fontSize: 10,
                              textTransform:
                                "uppercase",
                            }}
                          >
                            Base Rate
                          </Text>

                          <div
                            style={{
                              marginTop: 3,
                              fontSize: 15,
                              fontWeight:
                                700,
                            }}
                          >
                            {formatMoney(
                              selectedRoute.base_rate
                            )}
                          </div>
                        </div>
                      </Col>

                      <Col span={12}>
                        <div
                          style={{
                            padding: 10,
                            border:
                              "1px solid #f0f0f0",
                            borderRadius: 8,
                          }}
                        >
                          <Text
                            type="secondary"
                            style={{
                              fontSize: 10,
                              textTransform:
                                "uppercase",
                            }}
                          >
                            Status
                          </Text>

                          <div
                            style={{
                              marginTop: 6,
                            }}
                          >
                            <StatusTag
                              active={toBoolean(
                                selectedRoute.is_active
                              )}
                            />
                          </div>
                        </div>
                      </Col>
                    </Row>

                    <Descriptions
                      column={1}
                      size="small"
                    >
                      <Descriptions.Item label="Express">
                        <Tag
                          color={
                            selectedRoute.express_enabled ===
                            false
                              ? "default"
                              : "orange"
                          }
                        >
                          {selectedRoute.express_enabled ===
                          false
                            ? "Disabled"
                            : "Enabled"}
                        </Tag>
                      </Descriptions.Item>

                      <Descriptions.Item label="Same Day">
                        <Tag
                          color={
                            selectedRoute.same_day_enabled ===
                            false
                              ? "default"
                              : "magenta"
                          }
                        >
                          {selectedRoute.same_day_enabled ===
                          false
                            ? "Disabled"
                            : "Enabled"}
                        </Tag>
                      </Descriptions.Item>

                      <Descriptions.Item label="Updated">
                        {formatDate(
                          selectedRoute.updated_at
                        )}
                      </Descriptions.Item>
                    </Descriptions>

                    <Space
                      style={{
                        marginTop: 10,
                      }}
                    >
                      <Button
                        icon={
                          <SwapOutlined />
                        }
                        disabled={
                          getPickupId(
                            selectedRoute
                          ) ===
                            getDeliveryId(
                              selectedRoute
                            ) ||
                          Boolean(
                            rateMap[
                              `${getDeliveryId(
                                selectedRoute
                              )}:${getPickupId(
                                selectedRoute
                              )}`
                            ]
                          )
                        }
                        onClick={() =>
                          createReverse(
                            selectedRoute
                          )
                        }
                      >
                        Reverse
                      </Button>

                      <Button
                        onClick={() =>
                          toggleStatus(
                            selectedRoute
                          )
                        }
                      >
                        {toBoolean(
                          selectedRoute.is_active
                        )
                          ? "Disable"
                          : "Enable"}
                      </Button>
                    </Space>
                  </div>
                </>
              ) : (
                <div
                  style={{
                    padding: 60,
                  }}
                >
                  <Empty description="Click a route to view details" />
                </div>
              )}
            </Card>
          </div>
        </Col>
      </Row>
    </div>
  );
}